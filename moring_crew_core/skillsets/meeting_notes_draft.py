...
class MeetingNotesDraft(Skillset, BaseModel):
    ...
    async def generate_meeting_notes(self, conversation_records: List[Dict[str, str]], cached_contents: List[str]) -> Dict[str, str]:
        prompt = f"""
        Generate a meeting notes draft based on the following conversation records and cached contents using the provided template. Generate a Markdown version.
